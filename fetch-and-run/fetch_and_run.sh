...
if [ -z "${BATCH_FILE_TYPE}" ]; then
  usage "BATCH_FILE_TYPE not set, unable to determine type (zip/script) of URL ${BATCH_FILE_URL}"
fi
...
which curl >/dev/null 2>&1 || error_exit "Unable to find curl executable."
...
fetch_and_run_script () {
  ...
  curl -L "${BATCH_FILE_URL}" > "${TMPFILE}" || error_exit "Failed to download script"
...
fetch_and_run_zip () {
  ...
  local script="./${1}"; shift
  [ -r "${script}" ] || error_exit "Did not find specified script '${script}' in zip from ${BATCH_FILE_URL}"
